// CUDA Neural Network Engine — Python Bindings
...
public:
    ...
    void add_layer(const std::string& type, const std::vector<int>& args) {
        if (type == "Linear") {
            if (args.size() >= 2)
                model_.add<Linear>(args[0], args[1], args.size() > 2 ? args[2] : 1);
        } else if (type == "Conv2D") {
            if (args.size() >= 3)
                model_.add<Conv2D>(args[0], args[1], args[2],
                                   args.size() > 3 ? args[3] : 1,
                                   args.size() > 4 ? args[4] : 0,
                                   args.size() > 5 ? args[5] : 1);
        } else if (type == "BatchNorm2D") {
            if (args.size() >= 1)
                model_.add<BatchNorm2D>(args[0]);
        } else if (type == "MaxPool2D") {
            if (args.size() >= 1)
                model_.add<MaxPool2D>(args[0], args.size() > 1 ? args[1] : args[0]);
        } else if (type == "AdaptiveAvgPool2D") {
            if (args.size() >= 2)
                model_.add<AdaptiveAvgPool2D>(args[0], args[1]);
        } else if (type == "ReLU") {
            model_.add<ReLU>();
...
PYBIND11_MODULE(cnn_engine, m) {
    m.doc() = "CUDA Neural Network Engine";
    ...
    py::class_<Dropout, std::shared_ptr<Dropout>>(m, "Dropout")
    ...
    py::class_<Adam, std::shared_ptr<Adam>>(m, "Adam")
        .def(py::init<float, float, float, float, float>(),
             py::arg("lr") = 0.001f, py::arg("beta1") = 0.9f, py::arg("beta2") = 0.999f,
    ...
    py::class_<CrossEntropyLoss, std::shared_ptr<CrossEntropyLoss>>(m, "CrossEntropyLoss")
        .def(py::init<>())
        .def("compute", &CrossEntropyLoss::compute);

    py::class_<MSELoss, std::shared_ptr<MSELoss>>(m, "MSELoss")
        .def(py::init<>())
        .def("compute", &MSELoss::compute);
